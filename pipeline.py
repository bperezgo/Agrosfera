...
def main(JSON, optimization_path, output_path, iterations):
    ...
    try:
        ...
        subprocess.run(['del', '/f', '*.txt'], cwd = './agrosimulation/temp', shell = True)
    except FileNotFoundError:
        logger.info('temp in agrosimulation is empty')
    try:
        subprocess.run(['del', '/f', '*.csv'], cwd = './analyze/temp', shell = True)
        subprocess.run(['del', '/f', '*.txt'], cwd = './analyze/temp', shell = True)
    except FileNotFoundError:
    ...
    while i < iterations + 1:
        ...
        optimization_prov = optimization_path + str(i) + '.txt'
        output_prov = output_path + str(i) + '.csv'
        _executeSimulation(JSON, optimization_prov, output_prov, iterations)
        i += 1
...
def _executeSimulation(JSON, optimization_path, output_path, iterations):

    subprocess.run(['python', 'main.py', JSON, optimization_path, output_path, str(iterations)], cwd = './agrosimulation')
...
if __name__ == '__main__':
    ...
    main('./input/agro.json', 'temp/optim', 'temp/output', args.iterations)
